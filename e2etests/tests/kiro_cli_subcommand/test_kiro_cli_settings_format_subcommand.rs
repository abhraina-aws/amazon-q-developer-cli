...
fn test_kiro_cli_setting_format_subcommand() -> Result<(), Box<dyn std::error::Error>> {

    println!("\n🔍 Testing kiro-cli settings --format <FORMAT> <SETTINGS>... | Description: Tests the <code>kiro-cli settings --FORMAT <FORMAT> <SETTINGS> </code>subcommand to validate FORMAT content.");
    let response = q_chat_helper::execute_q_subcommand("kiro-cli", &["settings", "--format", "json-pretty", "chat.defaultAgent"])?;
    ...
    println!("✅ Kiro-cli settings format subcommand executed successfully!");
